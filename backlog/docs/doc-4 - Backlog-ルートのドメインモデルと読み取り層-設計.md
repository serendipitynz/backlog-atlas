...
- **解析不能** … frontmatter が YAML として読めない、ファイルそのものが読めない、または必須フィールドを欠くために当該ファイルをドメインモデルへ写せない事象。必須フィールドはタスクでは id・title・status、マイルストーン・文書・意思決定では id・title である（4 章、3.2）。当該 1 件を不整合表示にし、他ファイルの読み取りは続行する。任意フィールドの不在はこれに当たらない（4 章）。
  ...
  - **マイルストーン・文書・意思決定では、その種別の値を組み立てられない**。id が無い 1 件は一覧の並び順にも編集の宛先にもなれないためで、代わりに**写せなかったファイル**（1 章）として所在・種別・失敗理由だけを残す。**写せなかったファイルは解析不能からのみ生じる**（想定外スキーマまで到達した 1 件は id を持っており、一覧に並ぶ側にいる）。
...
- 非タスク管理ファイルの読取・解析失敗を表示モデルへ残すのは TASK-88（decision-24）。本章の解析不能・想定外スキーマの対象が管理ファイル 1 件へ広がり、写せなかったファイルが加わった。画面側の置き場は doc-10 §5・§6 が定める。**意思決定の画面上の置き場は TASK-118 が作る**（doc-10 §9）。
