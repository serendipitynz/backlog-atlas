...
- 同一 Backlog ルート更新時の競合検出・再読み込み（ファイル監視）は TASK-14。本層のドメインモデルはその再構築単位（ルート）と整合させる。**単位はルートで確定した**（decision-42。初版は「ルート／ファイル」と両論を置いていた）。
...
- **同じ id を持つ管理ファイルが 2 件あるときは、読み取り順（走査が返したパスの順）がそのまま残る。** 並べ替えが安定であることがこれを与えるので、比較規則の側に「同じなら何を見るか」の段を足さない。**画面は 2 件とも描く（TASK-190 ＋ TASK-174）。** 一覧列のカードを識別しているのは所在パスで、id はカードが載せる値のほうである — 内容由来の値を鍵にしていた間は、重複した鍵に対して Svelte が投げる例外で区画が丸ごと消えていた（開発時だけでなく製品ビルドでも。2026-08-14 に `svelte/src/internal/client/dom/blocks/each.js` を読んで確かめた）。**描けることと選べることは別である** — 選択・編集セッション・閲覧の対象は id で解決しており、同じ id の 2 件ではどちらのカードを押しても読み取り順で先に来たほうが開く。**その状態を本節は述べるだけで、直すのは TASK-205 である。** **読み取り層が並びを決められることと、画面が 2 件を並べられることは別である。**
