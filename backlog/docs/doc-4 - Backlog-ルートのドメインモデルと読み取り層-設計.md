...
破棄せず、可能な限りタスク単位で不整合表示へ落とす。事象を次の 4 つに分けて扱う。

- **解析不能** … frontmatter が YAML として読めない、または必須フィールド（id・title・status、4 章の分類）を欠くために当該ファイルをドメインモデルへ写せない事象。当該タスク 1 件を不整合表示にし、他タスクの読み取りは続行する。任意フィールドの不在はこれに当たらない（4 章）。
- **想定外スキーマ** … frontmatter は読めるが、値や構造がサポート範囲外（未知の status 値、未知の SECTION、存在時構造検査項目の構造破損など）である事象。判別できたフィールドは活かし、範囲外フィールドのみ不整合表示へ落とす（decision-2）。ただし draft の `Draft` は既知の status 値であり、未知 status として扱わない（3.4）。
...
**不整合表示** … 解析不能・想定外スキーマ・参照欠損を検知したとき、当該タスクを捨てず、判別できたフィールドと不足の明示に切り替えて画面へ出す表示状態（decision-2）。ドメインモデルのタスクは health（正常／縮退。3 章の表のまま）と不足内容を保持し、画面はそれを根拠に不整合表示へ切り替える。**旧称は「縮退表示」で、decision-22 が改称した**（指す状態は変えていない）。画面での見せ方 — カードには不整合の印 ⚠️ を 1 つだけ出し、由来名も族名も出さない — は decision-22 が定め、doc-7 §3・doc-8 §3・doc-11 が引く。**バージョン不整合（doc-9）も画面上は同じ不整合として束ねられるが、それは読み取り層の記録ではない** — 本章が定めるのは解析不能・想定外スキーマ・参照欠損の 3 つだけである。

**読み取り層の側の語は「縮退」のままである。** `DegradeEvent`・`TaskHealth` と wire のトークン（`unparseable`・`unexpectedSchema`・`danglingReference`）は改称していない。指示対象が「読み取り層が記録した 1 件の事実」であって「画面が束ねた状態」ではないためで、decision-22 がこの 2 つを分けている。

読み取り層は例外でルート全体の読み取りを止めない。1 ファイルの事象は 1 タスクの不整合に閉じ込め、ルート単位の致命はルート読取不能として分離する。
