...
- **1 件の値にカンマを含める操作**: 値をカンマ区切りの 1 値として読むオプション（`task create -l`・`task edit -a`・`task edit --add-label`／`--remove-label`・`task edit --depends-on`・`doc update --tags`）では、1 件のつもりの値がカンマの位置で分かれる。`task edit --add-label "a,b"` は labels に `a`・`b` の 2 件を書き、各要素の前後空白を落として空要素を捨てる（実測。`" c , d "` は `c`・`d`、`"e,,f"` は `e`・`f`）。**数える単位は CLI のオプションであって画面の欄ではない** — ラベル欄は 1 つだが 2 つのオプションへ分かれるので、画面で数えると 6 が 5 に見える。GUI は発行前に拒み、理由を述べる（doc-11 §5）。
- **カンマを含む既存のラベルを消す操作**: `--remove-label "x,y"` は、labels に `x,y` を 1 件持つタスクに対して終了コード 0 と `Updated` を出しながら 1 件も消さない（実測。値を `x`・`y` の 2 件へ分けたうえで、そのどちらもタスクに無いため。存在しないラベル名を渡したときと同じ結末であり、`-a ""`・`--ref ""`・`--depends-on ""` と同型の沈黙無変更である）。**この場合を持つのはラベルだけである** — assignee・依存・タグは非空全置換なので、カンマを含む既存要素は渡す集合から外せば消え、引数のどこにもカンマが現れない。ラベルだけが増減で発行され、消す値そのものが引数になる。GUI は発行前に拒み、理由を述べる（doc-11 §5）。**カンマを含むラベルは CLI では書けないので、この状態は管理ファイルの手編集から来る** — 一度書かれると CLI はそれを保つ（実測。別のラベル操作を通すと `- 'x,y'` と引用して保存し直す）。

**マイルストーン説明の更新を除き**、これらは decision-2 の「更新は Backlog CLI へ委譲」を保つ限り、CLI が提供するまで Atlas も提供しない。CLI 版が上がって対応サブコマンドが増えた場合に操作写像へ追加する（3 章末の版検査に従う）。**「CLI に手段が無い」ことは、それだけでは直接書き込み操作にする理由にならない** — decision-21 の 3 条件（CLI がその値を書く経路を現に持つ・frontmatter とファイル名に触れない・書く範囲が読み取り層の読む範囲と同一）をすべて満たすものだけが通る。上の各項でこれを満たすものは無い: 参照・依存・assignee・ラベルはいずれも frontmatter であり（条件 2 に反する）、文書の削除はファイルそのものを消す操作で、範囲という形を持たない。`milestone update` 相当が CLI に現れた場合は、説明の更新を CLI へ戻す（decision-21 の後続への影響）。
