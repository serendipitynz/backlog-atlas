...
- **カンマを含む既存のラベルを消す操作**: `--remove-label "x,y"` は、labels に `x,y` を 1 件持つタスクに対して終了コード 0 と `Updated` を出しながら 1 件も消さない（実測。値を `x`・`y` の 2 件へ分けたうえで、そのどちらもタスクに無いため。存在しないラベル名を渡したときと同じ結末であり、`-a ""`・`--ref ""`・`--depends-on ""` と同型の沈黙無変更である）。**この場合を持つのはラベルだけである** — 他の 6 つは渡す集合か 1 件ずつの値で全置換するので、カンマを含む既存要素は渡す値から外せば消え、引数のどこにもカンマが現れない。ラベルだけが増減で発行され、消す値そのものが引数になる。GUI は発行前に拒み、理由を述べる（doc-11 §5）。**カンマを含むラベルは CLI では書けないので、この状態は管理ファイルの手編集から来る** — 一度書かれると CLI はそれを保つ（実測。別のラベル操作を通すと `- 'x,y'` と引用して保存し直す）。
