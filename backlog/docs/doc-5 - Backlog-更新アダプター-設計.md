...
- **排出猶予**（decision-18）: **排出猶予**とは、子プロセスが消えたあと、その標準出力・標準エラーの読み切りを待ってよい上限時間を指す。値は 1 秒。パイプが終端に達するのは書込み側が 1 つ残らず閉じたときなので、子プロセスを終了させても終端が来るとは限らない（配管を継承した子孫プロセスが持ち続ける）。読み切りを待ち続けると、CLI 終了期限で取り除いた無期限の待ちがそこで戻る。子プロセスが自分で終了した場合は排出猶予まで待って読めた分を取り、期限到達で強制終了した場合は読み出しをまったく待たない。**1 回の起動が Atlas を止める時間の上限は「CLI 終了期限、または子プロセスの所要時間 ＋ 排出猶予」である。** 猶予を過ぎて届いた出力は失われ得るが、判断は終了コードであり出力はその理由文なので、掲げる保証は上限内に戻ることに留める。
