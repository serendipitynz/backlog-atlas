...
- **直接書き込みの失敗**（decision-21）: 直接書き込み操作は CLI プロセスを起こさないので CLI 失敗ではないが、**失敗の扱いは同じ**にする — 更新の不成立として扱い、失敗理由を利用者へ提示し、1 画面操作の以降の呼び出しを行わない。終了コードの代わりに書き込みが返す入出力の失敗を成否とする。**単独で行った直接書き込みの失敗は要再読込ではない** — 一時ファイル置換（decision-17）が、途中で失敗したときに旧ファイルをそのまま残すためである。先行する呼び出しがある場合に要再読込になるのは CLI 失敗と同じ。**説明の本文範囲を持たないファイル**（`## Description` 見出しが無い）への書き込みは、起動前に拒否する側に置く — 見出しごと作ることは書式の発明であり、decision-21 の条件 1 に反する。
