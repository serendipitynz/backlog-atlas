...
- **1 件の値にカンマを含める操作**: 値をカンマで分けて読むオプションでは、1 件のつもりの値がその位置で分かれる。**アダプターが出せるオプションを全部測った結果、分けるのは 7 つである**（実測）: `task create -l`、`task edit -a`／`--add-label`／`--remove-label`／`--depends-on`／`--ref`、`doc update --tags`。`task edit --add-label "a,b"` は labels に `a`・`b` の 2 件を書き、各要素の前後空白を落として空要素を捨てる（`" c , d "` は `c`・`d`、`"e,,f"` は `e`・`f`）。**数える軸は CLI の読み方であって、アダプターがカンマ連結しているかではない** — `--ref` は参照ごとに繰り返して渡すので連結は 1 か所も無いが、CLI は各値を分ける。**繰り返し可であることはどちらの側の根拠にもならない**: `--ac` は繰り返せてカンマを保ち（`--ac "first,second"` は 1 件）、`--ref` は繰り返せて分ける（どちらも実測）。**残るオプションは分けないか、利用者の打った文字を運ばない**（本文・title・`--path`・`--milestone` はカンマを保ち、`--status`・`--priority`・`--type` は固定候補なので終了コード 1 で拒み、AC の index・`--task-handling`・`--reassign-to`・`--no-update-tasks` は Atlas が組む値である。いずれも実測）。GUI は発行前に拒み、理由を述べる（doc-11 §5）。
