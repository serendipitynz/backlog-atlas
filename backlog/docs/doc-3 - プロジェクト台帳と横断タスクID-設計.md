...
- 形は `<slug>:<TASK-ID>`。`<TASK-ID>` は当該プロジェクトの `config.yml` の task_prefix に従うプロジェクト内 ID（既定は `TASK-N`）。
...
- 右辺 TASK-ID が当該プロジェクトの ID 形式に一致することを検査する。
- `:` を含まない入力は、現在文脈のプロジェクト内 TASK-ID とみなす（下記 5.3）。

### 5.3 表示

- 横断画面（プロジェクト別スイムレーン、横断検索結果など複数プロジェクトが混在する面）では常に `<slug>:<TASK-ID>` を表示する。
- 単一プロジェクトに絞った文脈（あるプロジェクトの詳細内など）では `TASK-ID` のみを表示してよい。
- 各プロジェクト内のコミットメッセージ・Pull Request では従来どおり `TASK-N` を使う（doc-2）。Atlas は横断表示のためだけに slug を前置し、正本側の ID 体系を変えない。
