...
**タグは素の `vMAJOR.MINOR.PATCH` でなければならず、workflow はそれを検査して落とす**（2026-08-24 に足した。
TASK-157、decision-44）。**理由は配布層の外にある** — Atlas 自身の 版の告知 は、公開されたタグと走っている
ビルドの版を比べる 3 つの数として読み、読めないタグについては**何も告知しない**。したがって
`v0.1.1+build.1` で公開すると、それは現に新しいリリースなのに、どの Atlas もそれを述べない。
**ここで落とすことが、その沈黙を「意図した挙動」から「起こりえない」へ変える。** build metadata と
prerelease 接尾辞のどちらも受けない。**検査の置き場が上の 4 つと同じ段なのは、どちらもタグの形が
下流を壊す話だからである。** 版の告知 側の受け口は decision-44 §4 が持つ。
