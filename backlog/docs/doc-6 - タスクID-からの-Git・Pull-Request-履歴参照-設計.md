...
- **参照手段に掛ける 3 つの上限**（decision-19）: remote ホスト種別ごとの参照手段は、どれも次の 3 つの下で呼ぶ。① **gh 照会期限** — 1 回の照会について応答を待つ上限の長さ（GitHub 分は 30 秒）。到達したら照会を終了させ、その Pull Request だけを参照不能（照会期限到達）とし、他の Pull Request の結果は保つ。② **履歴読取の取消** — 画面が「この結果はもう表示しない」を伝えたとき、走っている照会を終了させる。取消の対象は読取識別子（1 回の履歴読取呼出しを区別する値。画面が採番し、読取と取消の両方に載せる）で指す。③ **同時照会上限** — 1 回の履歴読取の中で同時に走ってよい照会の個数の上限（2）。上限を超える Pull Request は先行の完了を待つ。remote ホスト種別を足すときも、その参照手段はこの 3 つに載せる。
