...
**本層が起動する外部コマンドは `git` と `gh` の 2 つで、どちらも decision-29 の 外部コマンド解決の順序 で決める**（2026-08-13、TASK-156）。順序は 2 段で、①アプリ設定の外部コマンド指定（`git_cli`／`gh_cli`。実行ファイルの絶対パス）②プログラム名（`git`／`gh`）である。①は値があれば存在検査をせずそのまま使い、自動解決へ落とさない（decision-16 が `backlog` に定めた読み方と同じ）。空白のみの値は未設定として読む。**doc-5 §4 の実行ファイル解決の順序が持つ npm サブパッケージの段は、こちらには無い** — `git` も `gh` も npm で入らないので、置いても発火しない段になる。

**②が空振りしたときに何が起きるかは、2 つで違う。**`gh` の失敗は 6 章の参照不能として利用者に出る。**`git` の失敗は、doc-3 §3.2 の Git remote 有無属性判定では「remote 無し」として記録される** — したがって 5 章の判別が偽になり、6 章の関連解決が「対象なし」として静かに落ちる。**この非対称が、`git` にも外部コマンド指定を置く理由である**（decision-29）。落ちたことが利用者から見えないので、①が唯一の回復手段になる。
