...
**2026-08-06 追記（decision-21、TASK-65）**: 「更新のうち Atlas が起こすものは Backlog CLI へ
委譲する」に、Atlas 自身が書く例外が 1 つできた — マイルストーンの説明の本文範囲だけを
直接書き換える更新である。範囲・条件・書き込み方法は decision-21 が定める。反転したのは
「Atlas は管理ファイルへ書かない」の 1 点だけで、読み取りを直接解析とすることも、CLI に
写像先がある更新を CLI へ委譲することも、本決定のまま動いていない。

doc-2 の要請に従い、直接解析採用に伴う次の扱いを明示する。

- バージョン差：動作確認した Backlog のバージョン範囲を固定し、想定する
  frontmatter/SECTION スキーマを明記する。想定外スキーマは検知して不整合表示する。
  範囲の現在値は **decision-7 の最低バージョン要件以上・上限なし**（doc-4 §4、doc-5 §3）。
  値そのものを本決定に写さないのは decision-27 による — 写しがあると、要件を動かすたびに
  写しの数だけ書き換えが要る。本決定を書いた時点の値は v1.47.1 で、TASK-58 が
  2026-08-01 に v1.48.0 を実測して移した。
