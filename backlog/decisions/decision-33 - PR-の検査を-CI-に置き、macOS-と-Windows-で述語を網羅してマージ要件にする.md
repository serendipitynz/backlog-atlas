...
## Linux を外す判断の改訂 (2026-08-22、TASK-105)

**§3 の「Linux では走らせない」は `rust` ジョブについてのままである。改訂したのは、それが
ワークフロー全体の方針でもあるかのように読める点だけである。**

### 1. 何が変わったか

TASK-105 が `e2e` ジョブを ubuntu-24.04 に置いた。**したがって WebView の apt 一覧は 3 か所目を
得た** — §3 が「その 3 つ目が誰も数え直さない写しになる」として拒んだ、まさにその形である。

**拒んだ理由は取り下げない。写しは依然として誰も数え直さない。** 変わったのは天秤の反対側である。
§3 が 3 か所目と引き換えに買うと言っていたのは **Linux 固有の `return` 1 行のコンパイル**であり、
TASK-105 が 3 か所目と引き換えに買うのは **GUI E2E が自動で走ること そのもの**である。
**E2E は Linux 以外のどこでも走れない** — `tauri-driver` の README は Linux と Windows を対応と
挙げるが、Tauri アプリに届くのは Linux 側だけで、wry が自動操作を実装しているのは webkitgtk だけ
だからである（実測は decision-40）。

### 2. `rust` は Linux で走らせないままである

§3 の 2 つの理由は両方とも生きている。**述語の網羅**は 1 行も動いていない。**apt 一覧の写し**は、
`e2e` が 3 か所目を作ったことで `rust` の Linux 化が **4 か所目**を要求するようになり、むしろ
強まった。**「`e2e` が払ったのだから `rust` も払える」と読まない** — `e2e` が払うのは他に走る場所が
無いからで、`rust` が買うものは変わっていない。

### 3. マージ要件は 3 つのままである

§5 の 3 つ（`frontend`・`rust (macos-latest)`・`rust (windows-latest)`）に `e2e (ubuntu-24.04)` を
足さない。理由は decision-40 §5 が持つ。**Consequences の「job の `name` と ruleset の文字列が
結びついている」は 3 つについてのままで、4 つ目は結びついていない。**

### 4. 数え直す先

apt 一覧の置き場は **README の「ソースからのビルド」・`release.yml` の Linux 手順・`ci.yml` の `e2e`
ジョブ の 3 つ**である。doc-13 §3.4 の表と AGENTS 和英の ツールチェーン 節が同じ数を持つ。
**`e2e` だけが `webkit2gtk-driver` と `xvfb` を余分に入れる**ので、行ごとの比較はそこで終わる。
