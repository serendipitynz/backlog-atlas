...
2026-08-01 の実装報告書のセキュリティ節で、そこは
...
語と指示対象は TASK-98 の対応表 第 1 版で先に固定した。
...
測り方は、出荷形の応答ヘッダを当てたまま 6 画面を WebKit と Chromium で開き、違反の数を数える形である。
...
**これは読みではなく実測である**（2026-08-17。出荷形＝応答
ヘッダ、WebKit と Chromium で、`blob:` の画像 1 枚が描けるかを見た）。出荷値では `load` が上がって `naturalWidth` が 1、違反 0 件。
...
**17 種類の変異を当てて全部落ちることを確かめた**（設定値を 1 つずつ書き換えては試験を流す形。
**2026-08-17 に再測。初版は 14 種類だった**）: `null` へ戻す、10 本それぞれを落とす、`object-src` を `'self'` へ緩める、
