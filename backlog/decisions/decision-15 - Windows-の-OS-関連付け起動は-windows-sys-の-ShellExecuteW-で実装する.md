...
Drop で対応する `CoUninitialize` を呼ぶ。guard は `PhantomData<*const ()>` で `Send` を落とす —
下げる対象は *呼んだスレッドの* カウントなので、スレッドを跨いだ guard は別のスレッドのカウントを
下げてしまう。

**戻り値は 3 分岐で読む。** 成功／失敗の 2 分岐では足りない。中間の 1 つが、成功しなかったのに
`ShellExecuteW` が求める状態には在る、という場合だからである。分岐は `com_init` が持つ。

- `S_OK`・`S_FALSE` … カウントが上がったので `CoUninitialize` を負う（`ComInit::Owed`）。
- `RPC_E_CHANGED_MODE`（そのスレッドが既に *別の* アパートメントモデルで初期化されている）…
  カウントは上がっていないので何も返さない（返せば自分が作っていない初期化を下げる）。起動は
  続行する。`ShellExecuteW` が求めるのは初期化済みのアパートメントであり、それは在るからで、
  失敗し得るのは STA を明示的に要求するシェル拡張だけである。開くのを拒む方が悪い答えになる。
- それ以外（`E_INVALIDARG`・`E_OUTOFMEMORY`・`E_UNEXPECTED`）… そのスレッドはアパートメントを
  **持たない**。これはこの呼び出しが防ぐために在る状態そのものなので、起動へ進まず失敗を返す。

`com_init` を `cfg` の中ではなく HRESULT の純関数にしたのは、この述語が本変更のレビュー 2 ラウンド
とも指摘の当たった箇所だからである（1 度目は `CoUninitialize` が無いこと、2 度目は
`SUCCEEDED(hr)` が「アパートメントが無い」と「別種のものが在る」を同じに扱っていたこと）。
純関数なら macOS 上のテストが全分岐を押さえられる。`RPC_E_CHANGED_MODE` の値だけは
`windows_sys` の束縛を借りずに書き下し、束縛が在る環境では両者の一致をテストが確かめる。
