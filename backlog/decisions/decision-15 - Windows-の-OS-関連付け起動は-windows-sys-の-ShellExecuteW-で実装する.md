...
あることを前提としている。`commands::task_file_open` は `#[tauri::command(async)]` であり、
main thread ではなくランタイムのワーカースレッドで走るため、初期化を自前で行わなければ「COM を
必要としないハンドラのときだけ動く」呼び出しになる。戻り値は捨てる（初期化済みスレッドでは
`S_FALSE` が返って何も変わらない）。対応する `CoUninitialize` は呼ばない — スレッドは起動より
長く生き、シェルがまだそのアパートメントを使っている可能性がある。OLE1 DDE を切るのは、
`ShellExecute` が応答しない相手を待って止まり得る唯一の経路がそこだからである。
...
代償は `unsafe` の FFI 呼び出しを自前で持つことである。1 関数・2 呼び出しに閉じ、安全性の前提
（`lpFile` が NUL 終端であること、パスの生存期間が呼び出しを越えること、他の引数が null を
受け付けること）は `SAFETY` コメントに書いた。
...
- 費用・制約
  - `unsafe` の FFI が 1 関数入る。`editor` はこれまで `unsafe` を持たなかった。
