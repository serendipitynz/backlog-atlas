...
**数え直し（AC #5）**: 着手時 189 箇所・45 ファイル（起票時の 184 は行数と出現数を混ぜた値で、内訳も docs とコードで各 1 ずれていた）。作業後 **127 箇所・40 ファイル**。**引き上げで触るのは 166 → 95 箇所・39 → 30 ファイル**（実測註 82・実測基準版の宣言 7・MIN_VERSION の定義 1・decision-7 の要件本文 1・README 4）。TASK-152 の Description と AC へ内訳ごと記録した。

**この数はレビュー後に直した。** 初版は 122 → 100・実測註 87 と書いており、2 つ誤っていた — ①`screen-text.test.ts` の検査を足したのが計数の後だったため 5 箇所を数え漏らした ②**種別で分けた残余をそのまま数として使い、残余の中身を 1 件ずつ読んでいなかった**ため、版差記録 3（`update.rs` の `-l` フラグ繰り返し）と付随的な版表記 2（`Version::parse` の doc comment の parser 入力例、`TRANSITION_CONFIRM_QUESTION` の註にある却下初稿の引用）を実測註に数えていた。**これは 1R の [P2] と同型の見落ちである** — テスト側の `Version::parse("1.48")` は「parser の規則で床とは無関係」と気づいて直したのに、同じ関数の doc comment を見落ちた。ユーザーの問い（「上げようと上げまいとずっと残るものという理解で合っているか」）を受けて 92 件を 1 件ずつ読み直して分かった。**decision-27 の Context に 7 種類目（付随的な版表記）を足し、残余を数として使うときは中身を読むことを本文に書いた。**

**TASK-152 へ委譲した判断が 1 件ある**（decision-27 §4 の末尾、TASK-152 の AC #7）: 上げても挙動が変わっていなかった実測註を、範囲表記にするか据え置くか新しい版へ動かすか。現状の decision-27 は 3 つ目を前提に書いてあるが、2026-08-12 にユーザーが可能性を指摘したのでTASK-152 が判断して追記する。
