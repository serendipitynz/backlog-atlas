...
台帳操作の直列化が画面ごとに分かれていた。登録は画面自身の `registerBusy`、各エントリの操作は `busySlug` だけを見ていたため、両者を同時に発行できた。各コマンドは「自分が書いた台帳」を返すので、2 件が同時に飛ぶと応答順が書き込み順と一致しない: `ledger_remove` は境界の lifecycle lock を解放してから、200ms 窓でポーリングする監視スレッドの join を待つ。その待ち時間に発行された登録が先に書き込んで先に応答すると、先行していた削除の古い `LedgerResponse` が後着し、`applyLedger` が登録直後のエントリを `entries` から落とす。`projects.toml` には残るため、被害は「次の読み込みまで画面が台帳と食い違う」ことである。

修正は指摘箇所より一段上、shell に置いた。`applyLedger` の呼び出し元は 3 つの台帳操作と、スイムレーンの行並べ替え（`move`、`ledger_update` で台帳を書く）の 4 つあり、台帳管理画面が持つ busy 状態では 4 番目が競合したまま残る。`ledgerBusy` を 4 経路すべてが呼び出し全体で保持し、画面へ `busy` として渡して台帳を書く操作部品を無効化する。拒否は無効化をすり抜けたクリックへの最後の関門として残し、並べ替えは黙って捨てずに notice で報告する（行は目に見えて動かなかったのだから）。

「古い応答を破棄する」方式は採らなかった。発行順は台帳書き込み順の代理として健全ではない — 同時発行の 2 件は lifecycle lock をどちらの順でも取りうるため、先に発行された側が新しい状態になりうる — し、`LedgerResponse` に比較できる版が無い。発行時点で直列化すれば、応答順は構造として書き込み順に一致する。

## 学び

境界が書き込みを直列化していても、応答が直列化されるとは限らない。`ledger_remove` の「lock を解放してから監視スレッドを join する」という設計（TASK-33 のデッドロック回避そのもの）が、フロントエンドから見ると応答の追い越しとして現れる。画面側で「操作は 1 件ずつ」を保証する責務は、境界のロックがあることを理由に省略できない。

そしてその責務は、操作を発行する画面ではなく `applyLedger` を持つ層に属する。台帳を書く経路が 2 画面にまたがっていたため、画面内の busy 状態では塞げなかった。
<!-- SECTION:NOTES:END -->
