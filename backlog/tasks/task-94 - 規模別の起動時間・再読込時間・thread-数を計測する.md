...
4. **フロントエンドの再計算は律速ではない。** 20 ルート 4,000 タスクで 1.69ms、**同じ台帳・同じ
   タスク数の直列読取 569.7ms の 0.3%** である。品質評価書が挙げた 4 つの計測対象のうち、これだけ
   桁が違う。**初版はここを 20 ルート 20,000 タスクの再計算と 20 ルート 4,000 タスクの直列読取で
   比べており、規模の違う 2 つの比を「同じ規模」と書いていた**（1R の [P2]）。両半分の 200 タスク列を
   `scale.rs` の `ROOT_COUNTS` に揃えて、1 つの行から採れるようにした。
