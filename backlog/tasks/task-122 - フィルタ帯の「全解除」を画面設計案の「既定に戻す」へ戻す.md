...
- [x] #3 doc-12 §4.3 の「画面では 全解除 と呼ぶ」注記が現状に合わせて改訂されている
<!-- AC:END -->
...
**AC が名指しした場所は 2 点ずれていた。** ①AC #3 の「doc-12 §3」は誤りで、注記があるのは §4.3（§3 は 02 タスク詳細）。PR #76 の 1R [P3] を受けて AC 本文を §4.3 へ直した。②AC が挙げていない改訂先が 2 か所あり、doc-11 §2.4 と §8 がどちらも画面の文言として 全解除 を引いていた。

**1R [P2] は「誤った出典表示を直した注記が、逆向きの網羅主張で同じ形の誤りを作った」型だった。** doc-12 §4.3 の差し替え後の注記が「画面と `token.ts` だけが 全解除 と呼んでいた」と書いており、その「だけ」が偽だった — 本 PR が doc-11 を触っているのは、doc-11 §2.4 と §8 がどちらもこの控えを 全解除 と呼んでいたからで、`filter.ts`・`App.svelte`・`token.test.ts` も同じ語を持っていた。**誤った網羅主張を直すときに、別の網羅主張を書かない。** 列挙をやめて「画面・実装・doc-11 が 全解除 と呼んでいた」に改めた。
