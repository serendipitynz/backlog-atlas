...
- [ ] #2 decision-13 が挙げる項目を型として持ち、Tauri コマンド境界から読み書きできる
- [ ] #3 固定ヘッダの「設定」から設定画面を開き、変更した値が次回起動後も残る
- [ ] #4 列折畳み・行折畳み・行非表示は本ファイルへ保存しない（decision-13 の一時状態）
- [ ] #5 外部エディタ指定を設定として持ち、外部エディタ経路の起動指定が「アプリ設定 → $VISUAL → $EDITOR」の順で解決する（doc-8 §7）
- [ ] #6 ファイル不在・破損時は既定値で起動して画面を止めず、既定値で動いている旨を告げる（起動時の全ルート読み取りは設定にせず必須のまま保つ — doc-9 §3.2）
- [ ] #7 継続検出を切っている間、外部エディタ経路が開く前にその旨を伝え、詳細画面から当該ルートを再読込できる（doc-8 §7）
<!-- AC:END -->
