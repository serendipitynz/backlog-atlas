...
1R の [P2] で、消した 1 文が別の記述の前提だったことが分かったので同じ PR で直した。README は `PATH` 問題の直し方を「設定 → 外部コマンド」と日本語表記だけで案内しており、「画面は日本語のみです。」が消えた時点でそれは日本語 OS 以外の読み手に対して誤りになる。失敗の筋書きは実在の構成で、同じ README の Windows の段落が英語ロケールの実機で `.msi` を確かめた記録を持っている。両 README とも、その読み手の画面に出る見込みが高い語を先に置き、もう一方を括弧に入れる形にした（`README.ja.md` は 設定 → 外部コマンド を先に、`README.md` は Settings → External commands を先に）。括弧の中身は `src/lib/messages/en.ts` の `heading`・`externalCommandsHeading` と突き合わせてある。

同じラウンドの [P3] で「設定の行が 表示言語 を名指ししていない」も出たが、オーナーが 2026-08-20 に「見つけられない利用者はいない」として不要と判断した。README は名指ししない。
