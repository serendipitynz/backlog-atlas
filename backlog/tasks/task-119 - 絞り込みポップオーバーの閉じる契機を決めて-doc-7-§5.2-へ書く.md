...
閉じる契機を **4 つ**として書いた（「閉じる」ボタン・Escape・ポップオーバーの外側の押下・
「＋ 絞り込み」の再押下）。**着手時の見立ては 3 つだったが、実装を数え直して 4 つだった** —
外側の押下の判定範囲が anchor 全体（控えを含む）なので、控え自身での開閉は別の経路になっている。
併せて、閉じる契機に含まれないもの（値の選択・`addFilter` の押鍵）と、閉じ方が条件の適用に影響しない
ことも書いた。対応表は `_sandbox/handoff/referent-table-task-119.md`。
<!-- SECTION:NOTES:END -->
