...
## 目視（2026-08-04。ユーザーが実機 Tauri の画面を撮って提示。写真 4 枚）

PR 提示時に「測っていないもの」として挙げた 3 件のうち **2 件が解消し、1 件が未確認のまま残った**。

- **解消 (1) 実機 Tauri の WKWebView での実描画** — 3 図形とも描かれ、選択中のボタンが `--info` の枠に
  なることを確認した。
- **解消 (2) 中央モーダル・全面シングルビューの実幅での見出し行** — 3 配置とも**見出しは 1 行**で、
  識別子・操作群・「閉じる」が折り返さずに収まっている。ハーネスが 30rem の箱でしか測れなかった箇所を
  実機が埋めた。
- **未確認 (3) 破棄前確認を通る配置切替** — 本タスクが触っていない経路で AC にも無いため、依頼から外した。
  **TASK-86（モーダルの閉じる要求に未保存確認を通す）が同じ経路を扱う。**

**目視でしか取れなかったもの**: 最初の 3 枚は既定＝いま出ている配置だったため、下線と `--info` の枠が
同じボタンに重なっていた。**設定画面から既定だけを動かした 4 枚目**で、青枠（いま出ている配置）と
下線（次回開く配置）が**別のボタンに同時に立つ**姿と、補助文「次回起動時は「全面シングルビュー」で
開きます」が出る姿が確認できた。doc-8 §2.2 に書いた「2 つは別のことを述べ、別のボタンで同時に真に
なりうる」が実機で成立している。**配置の切替は選んだ配置をそのまま既定として保存する**（`App.svelte`
の `requestPlacement`）ので、既定 ≠ 現在 を作るには設定画面から既定を動かすしかない — 同型の状態を
目視へ回すときはこの手順を添える。

**下線が枠に見えていないこと**もユーザーが確認した（「十分視認できます」）。`inset` の影を実測で捨てて
独立要素にした判断は、実機でも正しかった。寸法（2px）と色（`--fg`）はこのままで確定とする。
