...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 Windows で OS 関連付け起動をシェルを介さず実装する（ShellExecuteW 相当）
- [ ] #2 依存追加はその選定理由と導入範囲を確認してから行う（AGENTS の依存ゲート）
- [ ] #3 & ^ %…% と空白を含むパスで、パスがそのまま単一の引数として渡ることを Windows 上で確認する
- [ ] #4 editor::association_launcher の Windows 分岐と NO_ASSOCIATION_LAUNCHER の無効化理由を除去する
<!-- AC:END -->
