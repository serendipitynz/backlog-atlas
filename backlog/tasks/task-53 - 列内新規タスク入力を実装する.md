...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 レーンセル末尾の入口から title だけを受け取り、task create を 1 回発行する
- [x] #2 未対応列には入口を置かず、置かない理由を短く示す（無効化したボタンを置かない）
- [x] #3 title が空のままでは発行できず、必須である理由を示す
- [x] #4 CLI 縮退時は理由付きで無効化し、読み取りは続く
- [x] #5 作成後に当該ルートを読み直し、新しいカードがその列に現れる
- [x] #6 列の作成時 status 候補が 1 件の列では、その原文 status を渡し、渡す値を入力欄に表示する（正準列名をそのまま渡さない）
- [x] #7 列の作成時 status 候補が 0 件の列には入口を置かず、作成時の初期値を決められない理由を示す
- [x] #8 列の作成時 status 候補が 2 件以上の列では status を候補からの選択にし、既定を config.yml の宣言順で先頭にする
- [x] #9 backlog init --defaults の 3 status 構成（In Review が未宣言）で、In Review 列に入口が出ないことを回帰試験で確かめる
<!-- AC:END -->
