...
- **日本語の Markdown では、閉じる `**` の直前が句読点で後に文が続くなら半角スペースを 1 つ置く。**
  ...
  **ただし両者は同じ集合ではない。規則が名指す形は、検査が押さえる形の 1 つである。**
  4 連の衝突（`**第一。****第二**とは`）は**この規則そのものの case であり、抜けている半角スペースが
  欠陥である。** 規則の外にあるのは、開き側が左隣接でない形と、太字になった範囲が著者の区切った範囲と
  食い違う形の 2 つである。よって含意は一方向で、**緑は規則充足の根拠になるが、赤が必ずこの規則の
  違反だとは言えない** — どの表明が落ちたかを読む。
  **開き側の形は、開き側の前にも閉じ側の後にも半角スペースを要する**（`には**「〜」**と書いた` は
  片方だけでは描画されない）。**本項が述べているのは閉じ側の半分だけで**、もう半分を規則にするかは
  decision-46 が開いたままにしている。
