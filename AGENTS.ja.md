...
- Backlog のタスク・文書・マイルストーンの更新は、対象プロジェクトを作業ディレクトリと
  する Backlog CLI 呼び出しへ委譲し、**CLI にその値を書くサブコマンドがある範囲では**
  管理対象の Markdown ファイルを直接変更しない。CLI を通すことが、書いたファイルを Backlog と
  Atlas の双方が読める状態に保つ手段である — 手で編集すると、どちらも読めないファイルが
  残りうる。**サブコマンドを持たない値が 2 つある — decision の本文とマイルストーンの説明で、
  この 2 つはファイルを編集して書く。** 範囲は次の 2 項が定める。**マイルストーンの側は
  2026-09-06 に所有者が決めた。** それまでこの規則はエージェントとしてのあなたに例外なく
  効き、その範囲を書くのは製品だけだった。
- **decision はこの列挙の外である。CLI にそれを書く手段が無いためである。**
  ...
  **タスクと文書には上の規則が例外なく効く。** マイルストーンの説明は、CLI が書けない
  もう 1 つの値であり、範囲は次の項が定める。**この読みは 2026-08-13 に所有者が確定した**
  （TASK-162）。書き残しているのは、列挙の沈黙がこれを根拠づけているのではないからである。
  **どちらの許可も沈黙から導いたものではない** — こちらは確定であり、マイルストーンの側は
  決定である — ので、3 つ目を沈黙から導き直す回は、この節の最後の項が禁じている手を打つ
  ことになる。
- **マイルストーンの説明 — 製品もエージェントも同じ 1 範囲を書く**（decision-21）。`milestone` に
  `update`／`edit` が無いのは v1.50.1 でも v1.51.0 でも同じで（後者は 2026-09-06 に実測。`rename` が
  `--due-date` を得たが、説明を運ぶものは依然として無い）、説明は作成時にしか設定できないうえ、
  作り直すと id が変わる。
  ...
  理由にならない。**エージェントも同じ 3 条件のもとで同じ範囲を書く**（2026-09-06 所有者）。
  違うのは手段だけで、製品の書き込みは doc-9 §4 の更新前検出を通す一時ファイル置換、
  エージェントのそれは通常のファイル書き込みである。
...
### CI で走り、セッションでは走らない 実 OS 通知の配送検査

**`pnpm test` の外の 3 本目である** — `pnpm run os-notify`、`scripts/os-notify/`（decision-43）。
**配送検査 とは、外部の書き込みから監視の束の受け口まで実 OS 通知が届いたことを、届かなかったことと
区別して述べる 1 本のテスト（`sync.rs` の `#[ignore]` 付き）を、呼ばれたプラットフォームの上で
明示実行することを指す。** 監視のそれ以外 — 管理パス判定・デバウンス規則・ルート全体の読み直しへの
倒し方 — は決定的なテストが持っており、そちらは watcher を要さない。

**したがって毎回の検証は `pnpm test`・`pnpm run check`・`pnpm run lint` の 3 本のままである。**
上の 2 本が 4 本目でないのと同じ理由で、これも 4 本目にしない。

**既定実行では skip し、この経路では失敗させる。** `cargo test` は飛ばす — 通知が届かない環境では、
どのコード変更でも直せないもので赤くなる。この経路は束が来ないときに落ち、**来た束が `Rescan` でも
落ちる** — Rescan はストリームが欠けたことを述べるので配送については何も述べず、Atlas が正しく倒れる
ことは決定的なテストが既に持っている。**届かない環境で skip へ自動的に落ちる判定は持たない** —
届かないことを届かないと述べるのがこの経路の成果であって、走れなかったことではない。

**入口はテストが 1 本走ったことも確かめる。** libtest のフィルタが何にも当たらない実行は 0 で終わり
（cargo 1.96.0 で実測）、cargo にそれを失敗にするオプションは無いので、テストを改名すると緑が何も
述べなくなる。**テスト名を変える回は `scripts/os-notify/run.mjs` の定数も変える** — 落とせないように
してあるのがそのガードである。

**CI は 3 か所でこれを走らせ、どれもマージ要件ではない。** 走るのは `os-notify (macos-latest)`・
`os-notify (windows-latest)` と、`e2e (ubuntu-24.04)` の最後の 1 段である。Linux が `e2e` ジョブに
乗るのは、**あちらでは WebView の apt 一覧を入れるまで crate がリンクできず、Linux の `os-notify` は
その一覧の 4 か所目になるからである。**

**どのプラットフォームで測ったかは decision-43 にあり、ここには写さない。** 測定の写しはランナーの
image が変わった瞬間に古びる。
...
- **`os-notify`**（macOS と Windows）— 上の 実 OS 通知の配送検査（decision-43）。**Linux は意図して
  この matrix に入れず、`e2e` ジョブの最後の 1 段として走らせる** — WebView の apt 一覧を 4 か所目に
  書かないためである。**`rust` に畳み込むこともしない** — あちらはマージ要件で、こちらは赤くなれること
  自体に意味がある側の検査である。
...
**`os-notify (macos-latest)`・`os-notify (windows-latest)` も入れていない（decision-43 §5）。** 理由は
同じ形で、赤くする相手がプラットフォームの通知の挙動だからである — 監視に触っていない Pull Request を
それで止めてはならない。**読まなければならない名前はもう 1 つではなくなった。** 読むことを課すものは
無い。**それがこの判断と decision-40 の判断の代償の全部である。**
...
- **日本語の Markdown では、閉じる `**` のアスタリスクの並び (run) の直前が句読点で、run の直後が
  空白でも約物でもないなら、半角スペースを 1 つ置く。**
  **これは `**` についての規則で、`*` についてではない。** 単一アスタリスクの強調も同じ形で失敗する
  （`*文です。*次` はそのまま出る）が、**本項も検査もそこへ届かない。** 検査を広げるのは只ではない —
  対になっていない `*` は失敗した区切りより **wildcard であることのほうが多く**、ツリーには
  `v*`・`kind:*`・`[a-z0-9-]*` などが 10 個ある（描画された `*…*` は 14 個で、壊れているものは 0 件。
  2026-09-07 実測）。
  閉じ側の区切りは right-flanking でなければならず、直前が `。` のとき、直後が空白か約物でなければ
  それを満たさない — つまり `**〜です。**Atlas` は太字にならず `**` がそのまま出るが、
  `**第一。**（補足）` は正しく太字になり、スペースを要さない。
  **判定は run で行い、`**` の隣 2 文字で行わない。** CommonMark は連続するアスタリスクを 1 本の
  run として読み、しかも `*` 自身が約物なので、`**` の隣を見ると
  `**第一。****第二**とは`（描画されない衝突）を取り逃がす。
  **置く場所は、自分が意図した閉じ側の終わりである。** 閉じ側と次の開き側が 1 本の run に融合して
  いるときはその**内側**になる（`**第一。** **第二**とは`）。**先頭 2 文字の後ではない** —
  `***第一。***次` でそれをやると `*<strong>第一。</strong> *次` になってイタリックが消え、
  run の後なら `<em><strong>第一。</strong></em> 次` で正しい。**範囲を決めるのは両隣の文字であって、
  直前だけではない** — 後半を落とすと `（` の前にスペースを求めることになり、それは decision-46 が
  168 件すべてで誤った直し方だと測った形である。
  **CommonMark は行末と本文末を空白として扱う**ので、`**文です。**` はそのどちらでも閉じて太字になる —
  **強調の中で文が終わる書き方が発火するのは、同じ行で語が続くときだけである。**
  描画される場所すべてに効く: README 和英と、Atlas が `markdown-it` で描くタスク・文書の
  本文（decision-25）。
  **この規則とそれ以上を押さえるのが `src/lib/emphasis-closing.test.ts` である。** 対象は
  ...
  1 件見つけた。
  **2 つの条件がそろって規則の範囲であり、その理由は decision-46 が持つ。**
  2026-09-06 まで本項は条件を付けずに半角スペースを求めており、その字義は描画が成功している
  3,872 箇所へ届いていた — **そのどれもスペースを入れて良くならない。** 直後が語の 1,049 件では
  語と助詞の間が割れ、直後が約物の 2,823 件では約物の前が空くからである。
  `**Ubuntu なら 24.04 以降**で` はその 1 つで、このままで正しい。**したがって検査が緑であることは、
  規則が守られていることの根拠になる** — そこが変わった点である。
  **ただし両者は同じ集合ではない。規則が名指す形は、検査が押さえる形の 1 つである。**
  **しかも両者は同じ種類のものではない。検査は入力の形を列挙しておらず、描画の結果に表明を掛けて
  いる。** この規則が防ぐのは、その表明が落ちる入力の 1 つの形である — 4 連の衝突
  （`**第一。****第二**とは`）もその形で、抜けている半角スペースが欠陥である。**検査だけが捕まえる
  入力を列挙しようとしない。重なるうえに漏れる。** **真部分集合であることは証人 1 つで足りる** —
  `には**「〜」** と書いた。` は閉じ側の後に半角スペースがあるので**この規則を満たしており**、
  それでも開き側が左隣接でないので描画されない。よって含意は一方向で、**緑は規則充足の根拠になるが、
  赤が必ずこの規則の違反だとは言えない** — どの表明が落ちたかを読む。
  **その証人は開き側の前にも半角スペースを要する**（片方だけでは描画されない）。**本項が述べているのは
  閉じ側の半分だけで**、もう半分を規則にするかは decision-46 が開いたままにしている。
