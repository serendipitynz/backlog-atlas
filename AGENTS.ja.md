...
入れる必要が無い。**Tauri が注入する引数は型で見分ける** (`AppHandle`・`State<'_, T>`) —
`app`・`state` という綴りでではないので、ハンドルの改名は何も壊さない。この一覧に無い注入型が
現れたら、必須キーを黙って落とすのではなくテストが落ちる。**Rust の引数名は比較ではなく変換する** —
マクロの既定が `rename_all = "camelCase"` であり、実装したのはその一方向だけなので、属性を
書いた回はフロントエンドを直すまでここが落ちる。そして**フロントが一度も呼ばないコマンドの集合は
