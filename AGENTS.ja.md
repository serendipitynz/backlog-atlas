...
- **日本語の Markdown では、閉じる `**` の直前が句読点で後に文が続くなら半角スペースを 1 つ置く。**
  ...
  **直前の文字についての条件は規則の範囲であり、その理由は decision-46 が持つ。**
  ...
  3,872 箇所へ届いていた — **そのどれもスペースを入れて良くならない。** 太字の範囲が句の途中で
  終わっているか、直後が約物かのどちらかだからである。`**Ubuntu なら 24.04 以降**で` はその 1 つで、
  このままで正しい。**したがって検査が緑であることは、規則が守られていることの根拠になる** —
  そこが変わった点で、規則と検査がいま同じ集合を名指している。
