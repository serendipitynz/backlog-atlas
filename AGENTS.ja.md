...
- **日本語の Markdown では、閉じる `**` の直前が句読点で後に文が続くなら半角スペースを 1 つ置く。**
  ...
  本文（decision-25）。
  **これを押さえるのが `src/lib/emphasis-closing.test.ts` である。** 対象は
  ...
  **ただし両者は同じ集合ではない。規則が名指す形は、検査が押さえる形の 1 つである。**
  検査は開き側が左隣接でない形（`には**「〜」**と書いた`）と 4 連の衝突も捕まえるが、**どちらも
  半角スペース 1 つで書く側が事前に避けられる形ではない。** よって含意は一方向で、**緑は規則充足の
  根拠になるが、赤が必ずこの規則の違反だとは言えない。**
