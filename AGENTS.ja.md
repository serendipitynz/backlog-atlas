...
このワークフローには、細部ではなく判断である点が 4 つある。取り消す変更を加えるなら、
その理由を書く。
...
- **`THIRD-PARTY-LICENSES.txt` がタグの木を述べているかを、ビルド前に確かめる。** 判定は
  ...
  3 つの成果物すべての欠陥であって、気づいたプラットフォームの欠陥ではない。
...
**バンドル内の複製を確かめてあるのは macOS だけである。** TASK-159 が手元でビルドし、
`Contents/Resources/` が `LICENSE` と `THIRD-PARTY-LICENSES.txt` をコミット済みの
ファイルとバイト同一で持つことを確認した（ワークフローも同じ確認をする）。
**Windows と Linux のバンドラがリソースをどこへ置くかは実測していないので、何も確認しない** —
推測で書いた検査は、検査自身が間違っているせいでリリースを止める。**通知はリリースへ
独立した成果物としても添付される。** どちらにせよ、通知なしで出荷されるプラットフォームは
無い。
