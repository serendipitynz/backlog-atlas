...
- **日本語の Markdown では、閉じる `**` の後に文が続くなら半角スペースを 1 つ置く。**
  ...
  **規則の字義はこの検査より広く、差は 3,454 箇所である** — 2026-08-19 の実測値である。
  ...
  根拠にならない** — そちらを押さえるものは無い。**2 つの読みのどちらを本項が意味するかは TASK-194 が
  確定する。** それまではどちらもこのリポジトリの答えではない: 字義を描画条件へ狭めず、通った検査を
  字義充足と読まない。**字義は見た目より守りにくい** — TASK-194 自身の本文は字義を意識して書いたうえで
  5 箇所違反しており、1 箇所は直後が `:` で、半角スペースを入れると組版が崩れる形だった。
