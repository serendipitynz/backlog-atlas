...
pub struct Task {
    ...
    pub project: String,
    /// Storage state from the scan directory, independent of `status` (§3.4, AC #2).
...
mod tests {
    ...
    fn task(id: Option<&str>, storage_state: StorageState, health: TaskHealth) -> Task {
        Task {
    ...
    fn cross_reference_by_id_resolves() {
        let m = model(
            vec![task(Some("TASK-1"), StorageState::Active, TaskHealth::Ok)],
            vec![Milestone {
                id: "m-1".into(),
                title: "impl".into(),
                description: None,
    ...
    fn storage_state_is_independent_of_status() {
        let mut done_in_tasks = task(Some("TASK-1"), StorageState::Active, TaskHealth::Ok);
        done_in_tasks.status = Some("Done".into());
        assert_eq!(done_in_tasks.storage_state, StorageState::Active);

        let mut todo_in_completed = task(Some("TASK-2"), StorageState::Completed, TaskHealth::Ok);
        todo_in_completed.status = Some("To Do".into());
        assert_eq!(todo_in_completed.storage_state, StorageState::Completed);
    }

    // AC #2: project comes from the scan root, not frontmatter.
    #[test]
    fn task_carries_project_and_storage_state() {
        let t = task(Some("DRAFT-1"), StorageState::Draft, TaskHealth::Ok);
        assert_eq!(t.project, "atlas");
        assert_eq!(t.storage_state, StorageState::Draft);
    }

    // AC #3: kind-derived Type and normal labels are separate fields; raw labels never mix.
    #[test]
    fn type_and_normal_labels_are_separated() {
        let mut t = task(Some("TASK-1"), StorageState::Active, TaskHealth::Ok);
    ...
    fn serializes_with_doc_field_names() {
        let t = task(Some("TASK-1"), StorageState::Active, TaskHealth::Ok);
