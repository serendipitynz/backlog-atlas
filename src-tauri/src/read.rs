...
/// default (measured on v1.50.1); a missing optional key must not make the root unreadable.
...
fn read_milestones(
    ...
    for path in list_dir(source, ScanDir::Milestones)? {
        ...
        // and nothing else (measured on v1.50.1) — so its only 存在時構造検査 is the body's, and
...
///
/// v1.50.1's `milestone add` writes a plain `## Description` heading rather than a SECTION pair
/// (measured 2026-08-22); the span function accepts either, as this did. The **read** accepts both
...
date_format: yyyy-mm-dd\n";
    ...
    /// A task file as Backlog v1.50.1 writes one.
...
---\n\n"
    ...
    fn a_task_outside_the_recognized_locations_keeps_an_indeterminate_storage_state() {
        // A flat archive/*.md — not produced by v1.50.1, but reads must not depend on the
...
<!-- AC:BEGIN -->\n- [ ] #1 fine\n",
    ...
    fn the_task_prefix_is_matched_case_insensitively() {
        ...
        // (measured on v1.50.1); a case-sensitive check would degrade every task in such a root.
        let mut source = MemorySource::new();
