...
- いま見ている管理ファイル（タスク・文書・マイルストーン・決定事項）を外部エディタで開く。
- 表示や絞り込みの好み、折り畳みと行の表示状態、窓の大きさを次の起動へ引き継ぐ。
...
入れたはずの道具が見つからないときは、**設定 → 外部コマンド**（英語表示では
Settings → External commands）で絶対パスを指定してください。
...
v0.2.0 は自分自身を更新しません。新しい版は
[Releases](https://github.com/serendipitynz/backlog-atlas/releases) で公開するので、取得して
入れ替えてください。

新しい版が出ているかは、Atlas が起動ごとに 1 回 GitHub へ問い合わせます。出ていれば ☰ に印が付き、
メニューの **リリースページを開く** の行がその版を告げます。問い合わせには `gh` を使うので、`gh` の
導入と認証が要ります（`gh api` はパブリックリポジトリでも token を求めます）。答えが得られなかった
ときは何も出しません。Releases を watch しておけば、いずれにせよ公開時に通知されます。
