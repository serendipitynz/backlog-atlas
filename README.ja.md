...
**デスクトップアプリは、シェルの `PATH` を必ずしも受け継がない。**Finder・Dock・ランチャーから
起動した Atlas が受け取るのは OS のセッションが渡す `PATH` で、macOS ではそこにバージョン管理
ツール (fnm・nvm・asdf) が `backlog` を置いたディレクトリが入っていないことがある — 同じマシンの
...
- 外部コマンド: Atlas が起動するが同梱しない道具 — `backlog`・`git`・`gh`・外部エディタ — は、
  まずアプリ設定の指定で解決し、無いときだけ自動解決へ進む。GUI 起動したアプリはそれらを持つ
  `PATH` を受け継がないことがあり、そのとき利用者が Atlas の中から到達できる手段がこの指定で
  ある (decision-29)。
