  ...
  <header class="heading">
    <div class="line">
      ...
      <!-- 前後移動 (doc-8 §2.2) は 1 行目の右端、配置切替の手前 (画面設計案 02。doc-12 §3)。
           ↑↓ のアイコンのみのボタン (doc-11 §2.4) で、名前は `aria-label` が全部持つ。
           **群の名前は `laneGroupLabel` だけが決める** (doc-8 §2.2): この 2 つと位置表示が同じ行に
           並ぶので、片方が「セル」と刷って片方が「未分類区画」と刷ると、同じ群を 2 つの語で呼ぶ
           ことになる。未分類区画はレーンセルではない (doc-7 §1)。端での無効化の理由は隣の位置表示と
