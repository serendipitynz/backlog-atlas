<script lang="ts">
  // ファイルパス 区画 (doc-8 §3・§7): 外部エディタ経路 の区画に残った 2 つ (decision-45 §8)。
  // **見出しは 2026-08-28 に「外部エディタで開く」から改まった** — 区画に無い操作を名乗っていたためで、
  // **経路の名は動いていない**（あちらは doc-8 §7 のもので、行はメニューにある）。
  // **起動の控えは 1 つも無い** —
...
<DetailSection title={t().taskDetail.filePathHeading} section="transitions" {layout}>
