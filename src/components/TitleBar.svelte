<script lang="ts">
  ...
  // height. Windows・Linux mount nothing here: the OS draws their bar and `App.svelte` writes the same
  // sentence into the window's own title instead. Which of the two a build is in is `OVERLAY_TITLE_BAR`
  // (`platform.ts`), and this component is never mounted for the other one.
  //
