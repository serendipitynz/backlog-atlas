 ...
 *
 * 押さえるのは、重複しうる値が届いたときに**その区画・画面が在ること**である。鍵式は markup にしか
 * 無く、値を作る側 — 読み取り層・`config.yml`・画面自身 — はどれも重複を拒まないので、鍵が壊れていても
 * `lib/*.test.ts` は緑のまま通る。管理ファイルの id についてだけは doc-4 §7 が決めており、残る 3 つの
 * 入口を決めた文書は無い。
 *
 * **区画ごとではなく入口ごとに case を置く。** 並ぶ 4 つは 2026-09-17 に `{#each}` 84 件を数え上げて
 * 得た入口で、該当した 12 か所はすべてこのどれかに乗る。新しい `{#each}` にここへ行が要るかも同じ問いで
 * 決まる — 鍵式の値を作る層が重複を拒むか、である。画面ごとの網羅は狙っていない。
 *
