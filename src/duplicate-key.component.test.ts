/**
 * 画面横断契約: decision-47 の規則が、それを破れる 4 つの入口すべてで守られていること。
 *
 * 鍵式は markup にしか無いので、どれが壊れていても `lib/*.test.ts` は緑のまま通る。case を区画ごとでは
 * なく入口ごとに置くのはそのためで、並ぶ 4 つは 2026-09-17 の数え上げが得た入口である。**画面ごとの
 * 網羅は狙っていない。**
 *
 * どの `it` も数と、押せる状態にあることだけを見る。重複した 2 件のうちどちらが選ばれるかは
 * TASK-205 が扱う。
 */
...
/** 入口 1 (decision-47) — 同じ id の管理ファイルを 2 件、所在パスだけ違えて与える。 */
...
/** 入口 2 (decision-47) — `#1` を 2 行持つ受入条件を与える。 */
...
/** 入口 3 (decision-47) — `statuses` に同じ名を 2 度持つ `config.yml` を与える。 */
...
/**
 * 入口 4 (decision-47) — 同じ問題文が 2 度返る状態を、押して作る。
 *
 * 管理ファイルを触らずに届く唯一の入口なので、操作だけで組む: `addAliasRow` が積む行は key が空なので、
 * 2 行で `aliasProblems` が同じ 1 文を 2 度返す。
 */
